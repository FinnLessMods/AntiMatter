...
package net.mcreator.antimatter.world.dimension;
...
import net.mcreator.antimatter.item.AntiMatterDementionItem;
import net.mcreator.antimatter.block.AntirackBlock;
...
public class AntiMatterDementionDimension extends AntimatterModElements.ModElement {
	@ObjectHolder("antimatter:anti_matter_demention_portal")
	public static final CustomPortalBlock portal = null;
	public AntiMatterDementionDimension(AntimatterModElements instance) {
		super(instance, 1);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(AntirackBlock.block.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("antimatter:anti_matter_demention"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("anti_matter_demention_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("anti_matter_demention_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("anti_matter_demention_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new AntiMatterDementionItem().setRegistryName("anti_matter_demention"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("antimatter:anti_matter_demention"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("antimatter:anti_matter_demention")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.PURPUR_BLOCK.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? Blocks.PURPUR_BLOCK.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.PURPUR_BLOCK.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
